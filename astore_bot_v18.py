# 🚀 БОТ МАГАЗИНА ASTORE v18.0 (РЕАЛЬНАЯ АВТОВЫДАЧА ЧЕРЕЗ FRAGMENT API)
# Фреймворк: aiogram 3.x | База данных: SQLite | Платёжка: FreeKassa | Доставка: Fragment Reseller API
...
# АКТУАЛЬНЫЕ ДАННЫЕ FREEKASSA (КАССА #75920)
FREEKASSA_MERCHANT_ID = "75920"
FREEKASSA_SECRET_1 = "[r0FSEr@?jWEPf%"
FREEKASSA_SECRET_2 = "YxWY2zClaHs*-cl"
FREEKASSA_API_KEY = "3b73b7b261f6401a57dc360a29bf245e"
...
def generate_freekassa_url(merchant_id: str, amount: int, secret_1: str, order_id: int, currency: str = "RUB") -> str:
...
async def verify_freekassa_payment(order_id: int) -> bool:
    ...
    sign_data = "|".join(str(payload[key]) for key in sorted(payload))
    payload["signature"] = hmac.new(
        FREEKASSA_API_KEY.encode("utf-8"),
            ...
            async with session.post(url, json=payload, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if res.get("type") == "success":
                        ...
                        for order in orders:
                            if (
                                str(order.get("merchant_order_id")) == str(order_id)
                                and int(order.get("status", 0)) == 1
                            ):
                                return True
    except Exception as e:
        logging.error(f"Ошибка проверки платежа через API: {e}")
...
async def create_order_and_send_invoice(event_msg, user_id: int, target_username: str, state: FSMContext):
    ...
    pay_url = generate_freekassa_url(FREEKASSA_MERCHANT_ID, price, FREEKASSA_SECRET_1, order_id)
    ...
    text = (
        ...
        "Нажмите кнопку <b>«Оплатить»</b>, совершите перевод и нажмите <b>«Проверить оплату»</b>."
    )
...
async def check_payment(callback: types.CallbackQuery):
    ...
    is_paid = await verify_freekassa_payment(order_id)
    ...
    if is_paid:
        ...
        if delivery_res.get("success"):
            ...
            await callback.message.answer(
                ...
                f"Администратор уже уведомлен и завершит выдачу вручную в течение пару минут!",
                parse_mode="HTML"
            )
...
# ==================== ЗАПУСК ====================
async def main():
    print("🚀 Магазин Astore v18.0 (С РЕАЛЬНЫМ FRAGMENT API) запущен!")
