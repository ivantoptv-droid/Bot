# 🚀 БОТ МАГАЗИНА ASTORE v20.0 — ANYPAY + FRAGMENT API
...
ANYPAY_TEST_MODE = True  # Для первого теста; после подключения проверки оплаты выключи
...
def generate_anypay_url(
    ...
    """
    AnyPay SCI (legacy anypay.io/merchant).

    Для проекта с алгоритмом подписи MD5 (как в кабинете на скрине)
    контрольная подпись формируется строго так:

        MD5(currency:amount:secret_key:merchant_id:pay_id)

    Пример:
        md5("RUB:71:SECRET:18279:1")

    В подпись НЕ входят desc, lang, success_url и fail_url.
    """
    amount_str = str(int(amount)) if float(amount).is_integer() else f"{float(amount):.2f}"
    sign_string = f"{currency}:{amount_str}:{secret_key}:{project_id}:{order_id}"
    sign = hashlib.md5(sign_string.encode("utf-8")).hexdigest()

    # Для старого SCI AnyPay достаточно передать эти параметры.
    # desc не включаем в запрос: так меньше шансов получить несовместимость
    # с текущей страницей merchant.
    params = {
    ...
    return f"https://anypay.io/merchant?{urlencode(params)}"
...
async def testpay_command(message: types.Message):
    ...
    Включается только при ANYPAY_TEST_MODE = True  # Для первого теста; после подключения проверки оплаты выключи.
    Пример: /testpay 123
    """
